"""Recherche des combinaisons exponentielles régularisées pour prédire `d_csds`."""
...
def load_and_prepare(file_path):
    ...
    # Créer la cible logarithmique utilisée par la régression exponentielle.
...
def build_exponential_model(ridge_alpha=1.0):
...
def has_exact_redundancy(feature_cols):
    """Écarte les ensembles de variables qui impliquent une redondance algébrique exacte."""
    fs = set(feature_cols)
...
def fit_and_evaluate_exponential(train_df, val_df, feature_cols, ridge_alpha=1.0, cv_folds=5):
    ...
    model = build_exponential_model(ridge_alpha=ridge_alpha)
    ...
    cv_scores_log = cross_val_score(model, X_train, y_train_log, cv=cv, scoring="r2")

    # Back-transform predictions to original d space
    y_train_pred_d = np.exp(y_train_log_pred)
    y_val_pred_d = np.exp(y_val_log_pred)

    y_train_true_d = train_df["d_csds"]
    y_val_true_d = val_df["d_csds"]
    ...
    return {
        ...
        "R2_cv_mean_log": cv_scores_log.mean(),
        "R2_cv_std_log": cv_scores_log.std(),

        # Metrics in original d-space
        "R2_train_d": r2_score(y_train_true_d, y_train_pred_d),
        "RMSE_train_d": rmse(y_train_true_d, y_train_pred_d),
        "R2_val_d": r2_score(y_val_true_d, y_val_pred_d),
        "RMSE_val_d": rmse(y_val_true_d, y_val_pred_d),
    }
...
def get_branch_signature(feature_list):
    """Construit une signature de branche pour répartir les modèles retenus."""
    """
...
def is_diverse_enough(feature_list, selected_models, similarity_threshold):
    """Vérifie qu'un nouveau modèle est assez différent des modèles déjà sélectionnés."""
    """
...
# 6) DEAP SETUP
# ============================================================
if not hasattr(creator, "FitnessMaxExp"):
    creator.create("FitnessMaxExp", base.Fitness, weights=(1.0,))

if not hasattr(creator, "IndividualExp"):
...
def setup_genetic_algorithm(train_df, val_df, all_features, cv_folds, selection_mode="log"):
    ...
    """
    selection_mode:
        - 'log' : rank models using metrics on log(d)
        - 'd'   : rank models using metrics back-transformed on d
    """
    ...
    def eval_features(individual):
        ...
        try:
            ...
            if selection_mode == "log":
                ...
                # Selection on original d scale
                # CV still comes from log-space because exponential regression is trained there
                score = (
                    result["R2_val_d"]
                    + 0.5 * result["R2_cv_mean_log"]
                    - MODEL_PARAMS["feature_penalty"] * len(feature_list)
...
for dataset_name, file_path in FILES.items():
    ...
    if best_models_log:
        summary_log_df = pd.DataFrame(best_models_log)
        summary_log_df = summary_log_df.sort_values(
            by=["Selection_Score", "R2_val_log", "R2_cv_mean_log"],
            ascending=False
        ).reset_index(drop=True)

        print("\nTOP MODELS - selection on log(d)")
        ...
        summary_log_df.to_csv(output_log, index=False)
        print(f"\nSauvegarde: {output_log}")

    # ========================================================
    # B) Exponential regression - selection in original d-space
    # ========================================================
    print("\n" + "=" * 100)
    ...
    if best_models_d:
        summary_d_df = pd.DataFrame(best_models_d)
        summary_d_df = summary_d_df.sort_values(
            by=["Selection_Score", "R2_val_d", "R2_cv_mean_log"],
            ascending=False
        ).reset_index(drop=True)
        ...
        output_d = REGRESSION_DIR / f"exp_regression_d_selection_{dataset_name.lower()}.csv"
        summary_d_df.to_csv(output_d, index=False)
        print(f"\nSauvegarde: {output_d}")
...
print("\n" + "=" * 100)
print("ANALYSE EXPONENTIELLE COMPLETE POUR TOUS LES DATASETS")
print("=" * 100)
