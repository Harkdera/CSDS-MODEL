"""Recherche des combinaisons polynomiales régularisées pour prédire `d_csds`."""
...
try:
    ...
    from methodologie_1.search_utils import build_random_individual, has_exact_redundancy_degree2, jaccard_similarity, is_diverse_enough, fit_and_evaluate_model
    ...
    from search_utils import build_random_individual, has_exact_redundancy_degree2, jaccard_similarity, is_diverse_enough, fit_and_evaluate_model
...
def fit_and_evaluate(train_df, val_df, test_df, feature_cols, degree=2, ridge_alpha=1.0, cv_folds=5):
    ...
    return {
        ...
        "R2_train": result["R2_train_target"],
...
def get_branch_signature(feature_list):
    """Construit une signature simplifiée pour limiter le nombre de modèles trop proches."""
    """
...
# 6) DEAP SETUP
# ============================================================
if not hasattr(creator, "FitnessMaxCSDS"):
    creator.create("FitnessMaxCSDS", base.Fitness, weights=(1.0,))

if not hasattr(creator, "IndividualCSDS"):
...
def setup_genetic_algorithm(train_df, val_df, test_df, all_features, cv_folds):
    ...
    def eval_features(individual):
        ...
        try:
            ...
            score = (
                result["R2_val"]
                + 0.5 * result["R2_cv_mean"]
                - MODEL_PARAMS["feature_penalty"] * len(feature_list)
                - MODEL_PARAMS["cv_std_penalty"] * result["R2_cv_std"]
            )
...
for dataset_name in SPLIT_FILES:
    ...
    print("\n" + "=" * 100)
    print(f"TOP 30 MODELES DIVERSIFIES (recherche genetique) - {dataset_name}")
    print("=" * 100)
    ...
    if best_models:
        ...
        summary_df = summary_df.sort_values(
            by=["Selection_Score", "R2_val", "R2_cv_mean"],
            ascending=False
        ).reset_index(drop=True)

        output_path = REGRESSION_DIR / f"d_csds_genetic_algorithm_diverse_{dataset_name.lower()}.csv"
        ...
        importance_path = REGRESSION_DIR / f"feature_importance_genetic_diverse_{dataset_name.lower()}.csv"
...
print("\n" + "=" * 100)
print("ANALYSE GENETIQUE DIVERSIFIEE COMPLETE POUR TOUS LES DATASETS")
print("=" * 100)
