...
from src.utils.paths import DESCRIPTIVE_STATS_RESULTS_DIR, INTERIM_DATA_DIR, PROCESSED_DATA_DIR
...
OUT_DIR = DESCRIPTIVE_STATS_RESULTS_DIR / "converged-csds"
GROUPS_OUT_ROOT = DESCRIPTIVE_STATS_RESULTS_DIR / "groups"
...
def run_eda_for_dataset(df: pd.DataFrame, dataset_label: str, output_dir: Path) -> Path:
    """Sauvegarde uniquement les statistiques descriptives pour un dataset CSDS."""
    df = df.copy()
    ...
    for col in numeric_cols:
        ...
        summary[col] = descriptive_stats(data_col)
